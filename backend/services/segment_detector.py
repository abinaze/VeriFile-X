...
def detect_segments(image_bytes: bytes, filename: str = "unknown") -> Dict[str, Any]:
    ...
    try:
        ...
        def _norm(vals: List[float]) -> np.ndarray:
            """
            Z-score each tile relative to the image's own mean/std, then
            squash through a sigmoid centered on a fixed anomaly threshold
            -- a tile only scores "hot" if it's a genuine statistical
            outlier relative to the rest of the image, not just the
            single most different tile in this particular photo.
            """
            a = np.array(vals, dtype=np.float64)
            mean, std = a.mean(), a.std()
            if std < 1e-10:
                # No meaningful variation between tiles -- nothing can be
                # a statistical outlier here.
